...
def _load_xyz_waveforms(pulse_dir: Path, case_id: int, is_driver_side: int, case_id_offset: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    xyz = np.stack([ax, ay, az], axis=0).astype(np.float32)  # (3,150)
    xy = xyz[:2, :].astype(np.float32)  # (2,150)
    return xyz, xy
...
def _load_waveforms_batch(
    ...
    n = case_ids.shape[0]
    x_acc_xyz = np.empty((n, 3, 150), dtype=np.float32)
    x_acc_xy = np.empty((n, 2, 150), dtype=np.float32)
    ...
    if np.any(ok_mask):
        ...
        x_acc_xyz = np.empty((0, 3, 150), dtype=np.float32)
        x_acc_xy = np.empty((0, 2, 150), dtype=np.float32)
...
def package_raw_packed(
    ...
    is_driver_side_all = x_att_raw_all[:, 11].astype(np.int64)
    ...
    np.savez(
        ...
        x_att_raw=x_att_raw.astype(np.float32),
        x_acc_xyz=x_acc_xyz.astype(np.float32),
        x_acc_xy=x_acc_xy.astype(np.float32),
        is_pulse_ok=is_pulse_ok.astype(bool),
        is_injury_ok=is_injury_ok.astype(bool),
        y_HIC=y_hic.astype(np.float32),
        y_Dmax=y_dmax.astype(np.float32),
        y_Nij=y_nij.astype(np.float32),
        ais_head=ais_head.astype(np.int64),
        ais_chest=ais_chest.astype(np.int64),
        ais_neck=ais_neck.astype(np.int64),
        mais=mais.astype(np.int64)
    )
...
def main():
    ...
    print(f"\n✅️ raw_packed (原始数值尺度, 未归一化) 已生成: {raw_path}")
